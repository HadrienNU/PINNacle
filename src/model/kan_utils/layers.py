...
class BasisKANLayer(torch.nn.Module):
    def __init__(
        ...
        self.stop_grid_update_iter = stop_grid_update_iter
    ...
    def forward_fast(self, x: torch.Tensor, step=0):
        if self.auto_grid_update and step < self.stop_grid_update_iter and step > 200:
    ...
    def forward_slow(self, x: torch.Tensor, step=0):  # TODO: ajouter forwad power, prodet fmean
        if (self.auto_grid_update and step < self.stop_grid_update_iter and step < 5000 and step > 6000):
        ...
        if original_shape[0] == 0:
            print("FULLY EMPTY X FOUND!!!!")
            #return torch.empty((0, self.out_features), device=x.device, dtype=x.dtype).requires_grad_()
            #x = x.new_zeros((0, self.out_features)).requires_grad_()
        id_ = torch.where(torch.isnan(torch.sum(self.weights, dim=1)) == True)[0]
        if id_.any():
            #raise RuntimeError("All outputs are NaN — training failed.")
            print("NAN FOUND IN WEIGHTS")
            print(self.weights)
...
class SplinesKANLayer(BasisKANLayer):
    def __init__(
        ...
        auto_grid_update=False,
        stop_grid_update_iter=50,
        auto_grid_allow_outside_points=0.15,
        auto_grid_allow_empty_bins=1,
        pooling_op="sum",
        pooling_args=None,
    ):
