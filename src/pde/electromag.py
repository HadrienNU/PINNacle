...
class Magnetism_2D(baseclass.BasePDE):
    ...
    def __init__(self, bbox=[-1, 1, -1, 1], space=[0, 1, 0, 1], circ=[-0.3, -0.3, 0.02], mu0=4*np.pi*1e-7, I=100.0, sigma=0.02):
        ...
        def boundary_space(x, on_boundary):
            ...
            return on_boundary and not np.isclose(np.linalg.norm(x - center_circ, axis=-1), self.circ[2]) #and not (x[0] > self.space[0] or x[1] > self.space[2])
...
class Electric_2D(baseclass.BasePDE):
    ...
    def __init__(self, bbox=[-1, 1, -1, 1], space=[0, 1, 0, 1], beam=[-0.4, -0.4], gamma=100.0, frequency = 1e12, Q=1e-9, eps0=8.854e-12, sigma_x=0.3, sigma_y=0.3):
        super().__init__()
        # output dim
        ...
        self.space = space
        self.beam = beam
        #self.geom = dde.geometry.Disk(space[0:2], space[2])
        #self.geom = dde.geometry.Ellipse(space[0:2], space[2], space[3])
        geom = dde.geometry.Rectangle(xmin=[bbox[0], bbox[2]], xmax=[bbox[1], bbox[3]])
        ngeom = dde.geometry.Rectangle(xmin=[space[0], space[2]], xmax=[space[1], space[3]])
        ...
        def rho_transverse(x):
            x, y = x[:, 0:1], x[:, 1:2]
            return (self.Q / (2 * np.pi * self.sigma_x * self.sigma_y)) * dde.backend.exp(-((x - self.beam[0])**2 / (2 * self.sigma_x**2) + (y - self.beam[1])**2 / (2 * self.sigma_y**2)))
